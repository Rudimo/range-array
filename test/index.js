...
describe('rangeArrayToString', () => {
	describe('validation errors', function() {
		it('arr should be Array', function(done) {
			expect(rangeArrayToString({})).to.be.rejectedWith(Error, 'validation error: arr should be Array').notify(done);
		});

		it('arr should be number', function(done) {
			expect(rangeArrayToString(['0'])).to.be.rejectedWith(Error, 'validation error: arr should be number').notify(done);
		});

		it('arr number should be positive', function(done) {
			expect(rangeArrayToString([0])).to.be.rejectedWith(Error, 'validation error: number should be positive').notify(done);
		});

		it('arr should be sort by asc', function(done) {
			expect(rangeArrayToString([1, 2, 3, 4, 6, 5, 7, 8])).to.be.rejectedWith(Error, 'validation error: arr should be sort by asc').notify(done);
		});
	});

	describe('check different values', () => {
		it('test', (done) => {
